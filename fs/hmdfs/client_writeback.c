...
/*
	struct timespec64 test_ts;
	long long begin_times, end_times, dura_times;
	ktime_get_real_ts64(&test_ts);
	begin_times = test_ts.tv_sec * 1000000 + test_ts.tv_nsec / 1000;// ms
	ktime_get_real_ts64(&test_ts);
	end_times = test_ts.tv_sec * 1000000 + test_ts.tv_nsec / 1000;
	dura_times = end_times - begin_times;
*/
const long long frequency_th_us = 200LL * 1000;
void hmdfs_writeback_inode_handler(struct work_struct *work)
{
	...
	// TODO: PLAN 1 - Sort the list every time the writeback work get queued.
   	// writeback_sort(hwb);
	// hmdfs_info("----- hmdfs_writeback_inode_handler -----");
	while (likely(!list_empty(&hwb->inode_list_head))) {
		...
		// if((cts.tv_sec * 1000000 + cts.tv_nsec / 1000) - (inode->i_mtime.tv_sec * 1000000 + inode->i_mtime.tv_nsec / 1000) < frequency_th_us) {
		// 	info->ab_frequency += 1;
		// 	if(info->ab_frequency == 1) info->ab_fre_ts = cts;
		// }
		// info->latest_wb_times += 1;
		iput(inode);
		spin_lock(&hwb->inode_list_lock);
		// hmdfs_info("----- SERVER? WRITE BACK FINISHED: 0/1: 0, ino: %lu -----", inode->i_ino);
	}
	// spc fix end:
	spin_unlock(&hwb->inode_list_lock);
}
