...
static const request_callback s_recv_callbacks[F_SIZE] = {
	...
	// spc fix: F_CPC_ATTR
	[F_CPC_ATTR] = hmdfs_server_cpc_attr,
};
...
int hmdfs_sendmessage(struct hmdfs_peer *node, struct hmdfs_send_data *msg)
{
	...
	// spc fix:
	spc_network_transceive(head->operations.command, 0, NULL);
	if (!node) {
...
// spc fix:
unsigned int g_hmdfs_reqrecv[F_SIZE];

void set_g_hmdfs_reqrecv(int id, int val) {
	g_hmdfs_reqrecv[id] = val;
}
int get_g_hmdfs_reqrecv(int id) {
	return g_hmdfs_reqrecv[id];
}
void add_g_hmdfs_reqrecv(int id, int val) {
	g_hmdfs_reqrecv[id] += val;
}
EXPORT_SYMBOL(set_g_hmdfs_reqrecv);
EXPORT_SYMBOL(get_g_hmdfs_reqrecv);
			      ...
			      struct hmdfs_head_cmd *head, void *buf)
{
	...
	// spc fix:
	if(head->operations.command != F_GETATTR && head->operations.command != F_READPAGE && 
		head->operations.command != F_WRITEPAGE && head->operations.command != F_ITERATE &&
		head->operations.command != F_SETATTR) {
		spc_network_transceive(head->operations.command, 1, NULL);
	}
	add_g_hmdfs_reqrecv(head->operations.command, 1);
			       ...
			       struct hmdfs_head_cmd *head, void *buf)
{
	...
	// spc fix:
	spc_network_transceive(head->operations.command, 2, NULL);
	if (command >= F_SIZE) {
		ret = -EINVAL;
		return ret;
	}
