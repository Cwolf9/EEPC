...
static void hmdfs_init_cmd_timeout(struct hmdfs_sb_info *sbi)
{
	...
	// spc fix: F_CPC_ATTR
	set_cmd_timeout(sbi, F_CPC_ATTR, TIMEOUT_COMMON);
}
...
static int hmdfs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	// spc fix start:
	if(strstr(sbi->local_src, "/account")) {
		hmdfs_info("----- spc_fix: Initializing Delegation List... -----");
		INIT_LIST_HEAD(&sbi->delegations);
		/* Initialize persistent cache information */
		err = hmdfs_init_pcache(sbi);
		if (err)
			goto out_freeroot;

		if (sbi->persist_cache_limit) {
			hmdfs_info("----- spc_fix: Client persistent file cache is enabled, limited to %llu -----", sbi->persist_cache_limit);
		} else {
			hmdfs_info("----- spc_fix: Client persistent file cache is not enabled -----");
		}

		err = spc_local_init(sbi);
		spc_info("spc_local_init: %d\n", err);
	}else {
		INIT_LIST_HEAD(&sbi->delegations);
		sbi->persist_cache_limit = 0;
	}
	// spc fix end:
