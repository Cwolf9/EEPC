...
// spc fix:
unsigned int g_hmdfs_send[F_SIZE];

void set_g_hmdfs_send(int id, int val) {
	g_hmdfs_send[id] = val;
}
int get_g_hmdfs_send(int id) {
	return g_hmdfs_send[id];
}
void add_g_hmdfs_send(int id, int val) {
	g_hmdfs_send[id] += val;
}
EXPORT_SYMBOL(set_g_hmdfs_send);
EXPORT_SYMBOL(get_g_hmdfs_send);
		    ...
		    __u8 file_type, struct hmdfs_open_ret *open_ret)
{
	...
	// spc fix: permission
	open_ret->permission = le32_to_cpu(resp->permission);
	spc_info("1 resp fid: %llu %lu", resp->file_ver, resp->file_id);
	spc_info("2 open_ret fid: %llu %lu", open_ret->fid.ver, open_ret->fid.id);
out:
	free_sm_outbuf(&sm);
	return ret;
}
...
// spc fix: F_CPC_ATTR
int hmdfs_send_cpc_attr(struct hmdfs_peer *con, const char *send_buf,
		    __u8 file_type, struct hmdfs_open_ret *open_ret)
{
	int ret;
	int path_len = strlen(send_buf);
	...
	add_g_hmdfs_send(F_CPC_ATTR, 1);
	hmdfs_init_cmd(&sm.operations, F_CPC_ATTR);
			  ...
			  struct page *page)
{
	...
	add_g_hmdfs_send(F_READPAGE, 1);
	// spc_network_transceive(); // page->mapping->host->i_ino
...
void spc_hmdfs_writepage_cb(struct hmdfs_writepage_context *ctx);
			      ...
			      struct hmdfs_writepage_context *ctx)
{
	...
	// spc fix start:
	struct inode *dvinode = ctx->page->mapping->host;
	struct hmdfs_sb_info *sbi = hmdfs_sb(dvinode->i_sb);

	if (info->permission != (FMODE_READ | FMODE_WRITE)) {
		spc_debug("info->permission: %d, %d %d", info->permission, FMODE_READ, (FMODE_READ | FMODE_WRITE));
        return -EACCES;
    }
	// spc fix end:
	...
	if (to_remote) {
		struct spc_client_writepage_request *spc_req;
		int dst_state = 1;
		uint32_t spc_did;
		...
		err = -1;
		dst_state = get_file_device_state(NULL, info, NULL, &spc_did);
		spc_info("device state of this file: %d", dst_state);
		// TODO: 暂时需要避免写文件之前的 open 操作唤醒了远端设备
		if((dst_state == 0 || (dst_state == 1 && pcache_mode == 2)) && (spc_coverage & 2) == 0) {
			spc_req = kmalloc(sizeof(struct spc_client_writepage_request), GFP_KERNEL);
			if(spc_req) {
				hmdfs_remote_fetch_fid(info, &fid);
				pfi = get_file_cache_by_fid(sbi, fid.ver, fid.id);
				pathlen = strlen(pfi->device_view_path);
				filepath = kzalloc(pathlen + 1, GFP_KERNEL);
				buf = kzalloc(HMDFS_PAGE_SIZE, GFP_KERNEL);
				if(filepath && buf) {
					memcpy(filepath, pfi->device_view_path, pathlen);
					spc_req->initiate_jif = jiffies;
					// 下层
					// spc_req->con = info->conn;
					// spc_req->param = ctx; // 唤醒回调函数 hmdfs_writepage_cb 中会自动 kfree()
					// spc_hmdfs_writepage_cb(ctx);
					// // return -EACCES; // 返回无权限错误 ? 然后延迟处理 ?

					data = kmap(ctx->page);
					memcpy(buf, data, HMDFS_PAGE_SIZE);
					kunmap(ctx->page);
					pos = (loff_t)ctx->page->index << HMDFS_PAGE_OFFSET;
					count = hmdfs_get_writecount(ctx->page);

					// 上层 TODO: 对同一个页的操作应该合并，不能依次执行，因为持久化文件没有更新，所以对同一页延迟的写入没有前缀性
					spc_req->filepath = filepath;
					// spc_req->page = ctx->page;
					spc_req->buf = buf;
					spc_req->pos = pos;
					spc_req->count = count;
					data = buf;
					err = 0;
					while(data && (*data) == 0 && err < HMDFS_PAGE_SIZE) {
						data ++;
						err ++;
					}
					spc_info("filepath, buf(*), count, pos: %s %s %d %d", spc_req->filepath, data, spc_req->count, spc_req->pos);
					err = spc_capture_client_request(spc_did, F_WRITEPAGE, spc_req);
					if(err == 1) {
						err = 0;
						req.private = ctx;
						req.data = NULL;
						resp.ret_code = 0;
						hmdfs_writepage_cb(con, &req, &resp);
					}else {
						err = 1;
					}
				}
				if(err) {
					if(filepath) kfree(filepath);
					if(buf) kfree(buf);
					kfree(spc_req);
				}
			}
			   ...
			   struct hmdfs_writepage_context *param)
{
	...
	add_g_hmdfs_send(F_WRITEPAGE, 1);
	// spc_network_transceive(); // param->page->mapping->host->i_ino
	hmdfs_init_cmd(&req.operations, F_WRITEPAGE);
	// spc fix start:
	if (con->sbi->persist_cache_limit) {
        hmdfs_write_pcache_page(param->fid.ver, param->fid.id, param->page, param->count);
    }
    // hmdfs_debug("---- spc_fix: send writepage request -----");
	// spc fix end:
			       ...
			       struct hmdfs_dcache_header *header)
{
	...
	// spc_network_transceive(); // file->f_inode->i_ino
	add_g_hmdfs_send(F_ITERATE, 1);
			      ...
			      const char *new_name, unsigned int flags)
{
	...
	// spc fix start:
	if (con->sbi->persist_cache_limit) {
        // hmdfs_debug("----- RENAME: old: %s, new: %s -----", old_name, new_name);
        ret = hmdfs_rename_pcache(con->sbi, old_path, old_name, new_path, new_name, flags);
        if (ret) {
            hmdfs_err("----- rename pcache failed: %d -----", ret);
        }
    }
	// spc fix end:
	ret = hmdfs_sendmessage_request(con, &sm);
		       ...
		       struct setattr_info *attr_info)
{
	...
	add_g_hmdfs_send(F_SETATTR, 1);
	// spc_network_transceive(); // send_buf
