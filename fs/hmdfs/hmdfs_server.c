...
struct file *hmdfs_open_path(struct hmdfs_sb_info *sbi, const char *path)
{
	...
	// spc fix :
	// hmdfs_info("hmdfs_open_path: %s %s", root_name, path);
	// /mnt/hmdfs/100/account/device_view/local/ /data/com.example.myapplnn/China160FPS720P.mp4
				 ...
				 unsigned int seq)
{
	...
	// spc fix start:
	hmdfs_file_release_device_delegations(conn);
	// spc fix end:
...
out_free:
	kfree(value);
	// spc fix start:
	err = strlen(file_name);
	if((file_name[err - 1] >= '0' && file_name[err - 1] <= '9') || file_name[err - 1] == 't') ret = 0;
	// spc fix end:
	return ret;
}
				       ...
				       struct open_response *resp)
{
    ...
    // hmdfs_debug("----- full path is %s -----", get_full_path(&info->file->f_path));
    delegation = hmdfs_file_get_delegation(con, info->file);
    if (!delegation) {
        delegation = hmdfs_file_alloc_delegation(con, info->file);
    }
	// spc fix end:
	...
	// spc fix start:
    resp->permission = cpu_to_le32(delegation->permission);
    spc_debug("device_id = %llu, permission: %d, fid: %llu %lu", con->device_id, delegation->permission, resp->file_ver, resp->file_id);
	// FMODE_READ 1  FMODE_WRITE 2
	// spc fix end:
	/*
...
// spc fix: F_CPC_ATTR
void hmdfs_server_cpc_attr(struct hmdfs_peer *con, struct hmdfs_head_cmd *cmd,
		       void *data)
{
	struct open_request *recv = data;
	int sizeread = sizeof(struct open_response);
	...
	info->file = hmdfs_open_path(con->sbi, recv->buf);
	...
	resp->file_ver = cpu_to_le64(hmdfs_server_pack_fid_ver(con, cmd));
	resp->file_id = 1234;
	// spc_network_transceive(); // info->file->f_inode->i_ino
	ret = hmdfs_sendmessage_response(con, cmd, sizeread, resp, 0);
	if (ret) {
		hmdfs_err("sending msg response failed, file_id %d, err %d",
			  info->file_id, ret);
			  ...
			  void *data)
{
	...
	// spc fix start:
	hmdfs_file_release_tmp_delegations(con);
	// spc fix end:
			   ...
			   void *data)
{
	...
	struct spc_request_info sri;
	sri.i_ino = file->f_inode->i_ino;
	spc_network_transceive(F_READPAGE, 1, &sri); // file->f_inode->i_ino
	...
	// 考虑将 close_ra 放到 struct readpage_request 中传过来
	// if(file->f_mapping->close_ra < 10) {
	// 	file->f_mapping->close_ra += 1;
	// 	// hmdfs_local_read_iter FFFFFFC0102D565C
	// 	// hmdfs_info("mapping_close_ra: 00 %d, inode: %lX; read_iter: %lx, mapping: %lX\n", file->f_mapping->close_ra, (long unsigned int)file->f_inode, (unsigned long)file->f_op->read_iter, (long unsigned int)file->f_mapping);
	// }
	// spc fix end:
...
out_response:
	sri.i_ino = filp->f_inode->i_ino;
	spc_network_transceive(F_ITERATE, 1, &sri); // filp->f_inode->i_ino
			    ...
			    void *data)
{
	...
	struct spc_request_info sri;
	sri.i_ino = file->f_inode->i_ino;
	spc_network_transceive(F_WRITEPAGE, 1, &sri); // file->f_inode->i_ino
			  ...
			  void *data)
{
	...
	struct spc_request_info sri;
	sri.i_ino = inode->i_ino;
	spc_network_transceive(F_SETATTR, 1, &sri); // recv->buf inode->i_ino dentry->d_inode
			  ...
			  void *data)
{
	...
	struct spc_request_info sri;
	sri.i_ino = inode->i_ino;
	spc_network_transceive(F_GETATTR, 1, &sri); // recv->buf inode->i_ino
				...
				struct hmdfs_head_cmd *cmd, void *data)
{
	...
	err = vfs_path_lookup(root_path.dentry, root_path.mnt, tmp_path, 0,
			      &path);
	if (err) {
		hmdfs_info("path found failed err = %d, con->sbi->real_dst: %s, tmp_path: %s", err, con->sbi->real_dst, tmp_path);
		goto free;
	}
