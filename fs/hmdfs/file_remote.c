				   ...
				   struct file *file, bool keep_cache)
{
	...
	if (info->remote_ino != open_ret->ino) {
		...
		ret = -ESTALE;
		// spc fix start:
		if (info->conn->sbi->persist_cache_limit) {
            hmdfs_set_pcache_truncate(info->conn->sbi, file);
        }
		// spc fix end:
		goto out;
	}
...
out:
	// spc fix: permission
	info->permission = open_ret->permission;
	trace_hmdfs_open_final_remote(info, open_ret, file, reason);
	if (ret)
		return ret;
...
int hmdfs_do_open_remote(struct file *file, bool keep_cache)
{
	...
	spc_info("3 open_ret fid: %llu %lu", open_ret.fid.ver, open_ret.fid.id);
	spc_info("4 info fid: %llu %lu", info->fid.ver, info->fid.id);
	err = hmdfs_open_final_remote(info, &open_ret, file, keep_cache);
	spc_info("5 info fid: %llu %lu", info->fid.ver, info->fid.id);
	// spc fix start:
	if (info->conn->sbi->persist_cache_limit) {
        spin_lock(&info->pcache_lock);
        spc_info("----- after open, init pcache: %s, file_ver = %llu , file_id = %u, hit: %d -----", file->f_path.dentry->d_name.name, info->fid.ver, info->fid.id, hmdfs_pcache_hit_at_lru(info->conn->sbi, file));
        if (!hmdfs_pcache_hit_at_lru(info->conn->sbi, file)) {
            err = hmdfs_page_pcache(info->conn->sbi, file, info->fid.ver, info->fid.id, NULL);
			err = 0;
        }
        spin_unlock(&info->pcache_lock);
    }
	// spc fix end:
out_free:
	kfree(send_buf);
...
// spc fix: F_CPC_ATTR
int hmdfs_do_cpc_attr(struct file *file) 
{
	struct hmdfs_inode_info *info = hmdfs_i(file_inode(file));
	struct hmdfs_peer *conn = info->conn;
	struct hmdfs_open_ret open_ret;
	...
	err = hmdfs_send_cpc_attr(conn, send_buf, file_type, &open_ret);
	if (err) {
		hmdfs_err("hmdfs_send_cpc_attr return failed with %d", err);
		goto out_free;
	}
	spc_info("hmdfs_do_cpc_attr fid: %llu %u", open_ret.fid.ver, open_ret.fid.id);
out_free:
	kfree(send_buf);
	return err;
}
...
int hmdfs_file_release_remote(struct inode *inode, struct file *file)
{
	...
	// F_RELEASE
	hmdfs_info("hmdfs_file_release_remote begin");
	inode_lock(inode);
	kref_put(&info->ref, hmdfs_do_close_remote);
	hmdfs_remote_keep_writecache(inode, file);
	inode_unlock(inode);
	hmdfs_info("hmdfs_file_release_remote end");
	return 0;
}
...
static int hmdfs_readpage_remote(struct file *file, struct page *page)
{
	...
	// spc fix start:
    if (info->conn->sbi->persist_cache_limit) {
        if (hmdfs_is_pcache_hit(info->conn->sbi, file, page)) {
			spc_info("----- Read local page! -----");
			if(pcache_mode == 1) {
				hmdfs_do_cpc_attr(file);
			}
            goto out_err;
        }
    }

	err = hmdfs_client_readpage(info->conn, &fid, page);
    ...
    if (info->conn->sbi->persist_cache_limit) {
        spin_lock(&info->pcache_lock);
		spc_info("----- Persist page in local! -----");
        err = hmdfs_page_pcache(info->conn->sbi, file, info->fid.ver, info->fid.id, page);
		err = 0;
        spin_unlock(&info->pcache_lock);
    }
				  ...
				  struct writeback_control *wbc)
{
	...
	file_open_root
	filp_open(filepath, O_RDWR, 0);

	hmdfs_write_pcache_page
	*/
	spc_info("sync_mode: %d, sync_all: %d, inode->i_ino: %lu", wbc->sync_mode, WB_SYNC_ALL, inode->i_ino);
	/*
	//  主动 write 时   sync_mode: 1, sync_all: 1, inode->i_ino: 4611686025287947399
	//  延迟 write 时   sync_mode: 0, sync_all: 1
