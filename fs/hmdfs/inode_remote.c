			...
			unsigned int flags)
{
	...
	struct hmdfs_peer *con = info->conn;
	struct hmdfs_inode_info *origin_info;
	...
	if (new_dentry->d_inode) {
        origin_info = hmdfs_i(new_dentry->d_inode);
        if (origin_info->permission != (FMODE_READ | FMODE_WRITE)) {
            return -EACCES;
        }
    }
    // hmdfs_debug("----- old_dentry_d_name is %s and new_dentry_d_name is %s -----", old_dentry_d_name, new_dentry_d_name);
...
static int hmdfs_setattr_remote(struct dentry *dentry, struct iattr *ia)
{
	...
	if ((ia->ia_valid & ATTR_SIZE) || (ia->ia_valid & ATTR_MTIME)) {
		...
		err = -1;
		dst_state = get_file_device_state(NULL, NULL, conn, &spc_did);
		spc_info("device state of this dentry(w): %d, send_buf: %s", dst_state, send_buf);
		if(dst_state == 0) {
			// err = spc_delay_setattr(spc_did, send_buf, &send_setattr_info);
		}
			 ...
			 struct hmdfs_getattr_ret **result)
{
	...
	err = -1;
	dst_state = get_file_device_state(NULL, NULL, conn, &spc_did);
	spc_info("device state of this dentry(r): %d, send_buf: %s", dst_state, send_buf);
	if(dst_state == 0 && (spc_coverage & 1) == 0) {
		err = spc_getattr(spc_did, send_buf, lookup_flags, attr);
		spc_info("spc getattr ret: %d, %llu %llu %llu", err, attr->stat.ino, attr->stat.uid.val, attr->stat.gid.val);
	}
	if(err != 0) {
		err = hmdfs_send_getattr(conn, send_buf, lookup_flags, attr);
		if(dst_state >= 0 && err == 0) {
			// 应该是记录远端文件的元信息，并以此返回，而不是用缓存文件的元信息返回；open完注意更新
			err = spc_record_getattr(spc_did, send_buf, lookup_flags, attr);
			spc_info("spc record getattr ret: %d, %llu %llu %llu", err, attr->stat.ino, attr->stat.uid.val, attr->stat.gid.val);
			err = 0;
		}
	}
	kfree(send_buf);
